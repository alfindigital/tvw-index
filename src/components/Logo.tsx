...
export function Logo({ size = "md", showWord = true, className = "" }: Props) {
  ...
  return (
    <div className={`flex min-w-0 items-center gap-2.5 ${className}`}>
      <div
        className={`${box} relative flex shrink-0 items-center justify-center bg-gradient-to-br from-primary to-[oklch(0.32_0.14_275)] text-primary-foreground shadow-[0_6px_18px_-6px_oklch(0.52_0.22_277_/_0.55)] dark:shadow-[0_6px_18px_-6px_oklch(0.66_0.22_278_/_0.65)]`}
        aria-hidden
      >
        <StackMark className={icon} />
        <span className="absolute inset-0 rounded-[inherit] ring-1 ring-inset ring-white/20 dark:ring-white/10" />
      </div>
      {showWord ? (
